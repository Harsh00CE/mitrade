...
const SellHistory = () => {
        ...
        const fetchOrders = async () => {
            try {
                ...
                if (json.success && Array.isArray(json.data)) {
                    const sells = json.data.filter((order) => order.type === 'sell')
                    setSellOrders(sells)
                }
            } catch (error) {
    ...
    return (
        <div className="p-4 sm:p-6 bg-gray-900 text-white min-h-screen w-full">
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-red-400 mb-6">Sell Order History</h2>

            {loading ? (
                <p className="text-gray-300">Loading...</p>
            ) : sellOrders.length === 0 ? (
                <p className="text-gray-400">No sell orders found.</p>
            ) : (
                <>
                    {/* Desktop Table */}
                        ...
                        {sellOrders.map((order) => (
                            <div key={order._id} className="bg-gray-800 p-4 rounded-lg shadow-md space-y-2">
                                <div className="flex justify-between text-sm">
                                    <span className="text-red-400 font-semibold">{order.symbol}</span>
